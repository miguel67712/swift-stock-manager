...
export default function POS() {
  ...
  const handleCheckout = async (method: "cash" | "card" | "mobile") => {
    ...
    } catch (err: any) { toast.error(err.message || "Échec du paiement"); }
  };
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-foreground flex items-center gap-2"><ShoppingCart className="h-4 w-4" />Vente en Cours</h2>
          {cart.length > 0 && <button onClick={clearCart} className="text-xs text-destructive hover:underline font-medium">Vider</button>}
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
              <ShoppingCart className="h-10 w-10 mb-3 opacity-20" /><p className="text-sm">Panier vide</p>
            </div>
          ) : cart.map((item) => (
      ...
      <div className="border-t border-border p-4 bg-card space-y-2">
        <div className="flex justify-between text-sm text-muted-foreground"><span>Sous-total</span><span className="font-mono">{formatXAF(cartSubtotal)}</span></div>
        <div className="flex justify-between text-sm text-muted-foreground"><span>TVA (10%)</span><span className="font-mono">{formatXAF(cartTax)}</span></div>
        <div className="flex justify-between text-lg font-bold border-t pt-2"><span>Total</span><span className="font-mono">{formatXAF(cartTotal)}</span></div>
        <Button className="w-full h-12 text-base font-bold mt-2" disabled={cart.length === 0 || createSale.isPending} onClick={() => setCheckoutOpen(true)}>{createSale.isPending ? "Traitement..." : "Checkout"}</Button>
      </div>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-56px)] md:h-screen overflow-hidden">
      <div className="flex-1 flex flex-col min-w-0 border-r border-border">
        <div className="p-3 sm:p-4 border-b border-border bg-card">
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Rechercher produit ou code-barres..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 h-10 sm:h-11 bg-background" /></div>
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
            <button onClick={() => setSelectedCategory(null)} className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${!selectedCategory ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>Tout</button>
        ...
        <div className="flex-1 overflow-auto p-3 sm:p-4">
                ...
                return (
                  <motion.button key={product.id} custom={i} variants={productCardVariants} initial="hidden" animate="visible" exit={{ opacity: 0, scale: 0.8 }} layout
                    ...
                    <div className="flex items-end justify-between mt-2 sm:mt-3">
                      ...
                      <span className={`text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded ${isOut ? "stock-critical" : isLow ? "stock-warning" : "stock-good"}`}>{isOut ? "RUPTURE" : `${product.quantity} dispo`}</span>
                    </div>
                  </motion.button>
                );
              })}
          ...
          {filteredProducts.length === 0 && <div className="flex flex-col items-center justify-center py-20 text-muted-foreground"><Search className="h-10 w-10 mb-3 opacity-30" /><p className="text-sm">Aucun produit trouvé</p></div>}
        </div>
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Mode de Paiement</DialogTitle><DialogDescription>Total: <span className="font-mono font-bold text-foreground">{formatXAF(cartTotal)}</span></DialogDescription></DialogHeader>
          <div className="grid grid-cols-3 gap-3 py-4">
            {[{ method: "cash" as const, icon: Banknote, label: "Espèces" }, { method: "card" as const, icon: CreditCard, label: "Carte" }, { method: "mobile" as const, icon: Smartphone, label: "Mobile Money" }].map(({ method, icon: Icon, label }) => (
      ...
      <Dialog open={!!receiptSale} onOpenChange={() => setReceiptSale(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader><DialogTitle className="flex items-center gap-2 text-success"><CheckCircle2 className="h-5 w-5" />Vente Complétée</DialogTitle><DialogDescription>Transaction enregistrée</DialogDescription></DialogHeader>
          {receiptSale && (
            <div className="space-y-4 py-2">
              ...
              <div className="border-t pt-2 space-y-1">
                <div className="flex justify-between text-sm text-muted-foreground"><span>Sous-total</span><span className="font-mono">{formatXAF(Number(receiptSale.subtotal))}</span></div>
                <div className="flex justify-between text-sm text-muted-foreground"><span>TVA</span><span className="font-mono">{formatXAF(Number(receiptSale.tax))}</span></div>
                <div className="flex justify-between font-bold"><span>Total</span><span className="font-mono">{formatXAF(Number(receiptSale.total))}</span></div>
              </div>
              <div className="text-center"><Badge variant="secondary" className="capitalize">{receiptSale.payment_method}</Badge></div>
              <Button variant="outline" className="w-full" onClick={() => setReceiptSale(null)}>Fermer</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
