    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div className="block sm:hidden space-y-3">
        {filteredProducts.map((product) => {
          ...
          return (
            <Card key={product.id} className={`shadow-card ${product.quantity === 0 ? "border-destructive/30" : product.quantity <= product.min_threshold ? "border-warning/30" : ""}`}>
                ...
                <div className="flex items-start justify-between">
                  ...
                  {isAdminOrManager && (
                    <div className="flex gap-1 ml-2">
                      <button onClick={() => openEditDialog(product)} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"><Pencil className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setDeleteConfirm(product.id)} className="rounded-md p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  )}
                </div>
      ...
      <Card className="shadow-card hidden sm:block">
        <CardContent className="p-0">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {filteredProducts.map((product) => {
                  const status = getStockStatus(product);
                  return (
                    <motion.tr key={product.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`hover:bg-muted/30 transition-colors ${product.quantity === 0 ? "bg-destructive/5" : product.quantity <= product.min_threshold ? "bg-warning/5" : ""}`}>
                      ...
                      {isAdminOrManager && (
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => openEditDialog(product)} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                            <button onClick={() => setDeleteConfirm(product.id)} className="rounded-md p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                          </div>
                        </td>
                      )}
                    </motion.tr>
          ...
          {filteredProducts.length === 0 && (
            <div className="py-16 text-center text-muted-foreground"><Package className="h-10 w-10 mx-auto mb-3 opacity-30" /><p className="text-sm">No products found</p></div>
          )}
        </CardContent>
      </Card>
