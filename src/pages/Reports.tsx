...
export default function Reports() {
  ...
  const statCards = [
    { label: "Revenu Total", value: formatXAF(totalRevenue), icon: DollarSign, color: "text-success", bg: "bg-success/10" },
    { label: "Transactions", value: sales.length.toString(), icon: BarChart3, color: "text-primary", bg: "bg-primary/10" },
    { label: "Moy. Transaction", value: formatXAF(avgTransaction), icon: TrendingUp, color: "text-primary", bg: "bg-primary/10" },
    { label: "Valeur Inventaire", value: formatXAF(inventoryValue), icon: Package, color: "text-warning", bg: "bg-warning/10" },
  ];

  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      <div><h1 className="text-xl sm:text-2xl font-bold text-foreground">Rapports & Analyses</h1><p className="text-sm text-muted-foreground mt-1">Statistiques de ventes</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <Card className="shadow-card"><CardHeader className="pb-2"><CardTitle className="text-base font-semibold flex items-center gap-2"><Award className="h-4 w-4 text-warning" />Top Produits</CardTitle></CardHeader><CardContent>
          {topProducts.length > 0 ? <div className="h-[260px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={topProducts} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="hsl(220 13% 91%)" /><XAxis type="number" tickFormatter={(v) => `${Math.round(v/1000)}k`} tick={{ fontSize: 11 }} /><YAxis type="category" dataKey="name" width={100} tick={{ fontSize: 10 }} /><Tooltip formatter={(v: number) => [formatXAF(v), "Revenue"]} /><Bar dataKey="revenue" fill="hsl(140 50% 30%)" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer></div> : <p className="py-12 text-center text-sm text-muted-foreground">Pas de données</p>}
        </CardContent></Card>

        <Card className="shadow-card"><CardHeader className="pb-2"><CardTitle className="text-base font-semibold">Ventes par Catégorie</CardTitle></CardHeader><CardContent>
          {categoryData.length > 0 ? <>
            <div className="h-[260px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={categoryData} cx="50%" cy="50%" innerRadius={50} outerRadius={85} paddingAngle={3} dataKey="value">{categoryData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip formatter={(v: number) => [formatXAF(v), "Revenue"]} /></PieChart></ResponsiveContainer></div>
          ...
          </> : <p className="py-12 text-center text-sm text-muted-foreground">Pas de données</p>}
        </CardContent></Card>
      </div>

      <Card className="shadow-card"><CardHeader className="pb-3"><CardTitle className="text-base font-semibold flex items-center gap-2"><Users className="h-4 w-4 text-primary" />Performance Caissiers</CardTitle></CardHeader><CardContent>
        <div className="overflow-x-auto"><table className="w-full text-sm min-w-[400px]"><thead><tr className="border-b">
          <th className="pb-3 text-left text-xs font-medium text-muted-foreground uppercase">Caissier</th>
          <th className="pb-3 text-right text-xs font-medium text-muted-foreground uppercase">Transactions</th>
          <th className="pb-3 text-right text-xs font-medium text-muted-foreground uppercase">Revenu</th>
          ...
          {cashierData.length === 0 && <tr><td colSpan={3} className="py-8 text-center text-muted-foreground text-sm">Pas de données</td></tr>}
        </tbody></table></div>
      </CardContent></Card>
    </div>
  );
